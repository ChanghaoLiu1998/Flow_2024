function cmplx = musical_iso_forman(cmplx,BO_1)
% -------------------------------------------------------------------------
% musical_iso.m
% -------------------------------------------------------------------------
...
for i = 1:cmplx(1).num(1).val
    ...
    A = cmplx(2).bfvol(cmplx(1).bndop(2).indx(i,1:p)).*...
...
cmplx(2).flat = kron(speye(cmplx(2).num(2).val),ones(1,3))*...
    spdiags(reshape(cmplx(2).bfvol'.*cmplx(2).dir',3*cmplx(2).num(2).val,1),0,...
